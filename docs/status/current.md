...
- explicit presentation editor domain mapping and revision conflict recovery, with one typed question draft shared by the inspector and editor canvas so unsaved authoring state is previewed without duplicating ownership;
...
3. **Frontend modularization:** continue TypeScript/module migration through
   legacy presentation/editor surfaces while preserving editor/live correctness. Question authoring now has typed domain draft/validation ownership plus live canvas projection from that same draft; the next editor slice is remaining content/design/audio inspector convergence and preview alignment. Identity auth composition is split into a TSX route, focused module UI and dedicated Google/verification hooks; reports and the manager presentation list use the shared Query cache, and live transport/runtime/React ownership is under `modules/live` with the stateful runtime extracted from React.
