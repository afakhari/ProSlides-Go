...
| P2 | Question and content authoring now use typed domain drafts shared by their inspectors and editor canvases. Unsaved authoring state projects immediately into theme/contrast-safe previews; discard returns to persisted server state and stale-revision conflicts preserve local drafts until explicitly resolved. Design/audio still contain larger custom draft/dirty logic. | Extend the same domain-owned pattern to design/audio where it reduces real complexity, then converge those previews without turning the editor into one giant form. |
...
| P2 | Browser coverage now includes question-authoring save/reorder/Persian-number/edit-conflict recovery plus content-authoring live preview/discard/save/conflict recovery on the real API stack, but focused component/API-state coverage is still thinner than protocol coverage. | Add Vitest + Testing Library + MSW for dense pending/success/validation/conflict/cancellation states where browser tests would be too slow or broad. |
