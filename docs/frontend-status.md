...
| P2 | Question, content, presentation design and presentation audio use typed domain drafts with explicit baseline/dirty/save/discard ownership. Editor panel confirmation, unsaved-browser protection, transient notices, responsive overlay locking, slide creation/deletion and type conversion are delegated to focused typed models/hooks; the route no longer calls slide mutation APIs directly. | Finish the remaining reorder/selection boundary, then convert the thinner editor route to TSX without duplicating draft ownership or creating a giant form. |
