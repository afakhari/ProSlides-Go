...
| P2 | Question, content, presentation design and presentation audio use typed domain drafts with explicit baseline/dirty/save/discard ownership. Panel confirmation, unsaved-browser protection, notices, responsive overlays, slide creation/deletion/type conversion, selection and reorder persistence are delegated to typed models/hooks; SlideList is typed presentational UI with no repository dependency. | Convert the substantially thinner editor route to TSX and finish small legacy editor UI cleanup without duplicating draft ownership or creating a giant form. |
