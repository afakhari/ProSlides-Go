...
| P2 | Question, content, presentation design and presentation audio now use typed domain drafts with explicit baseline/dirty/save/discard ownership. Audio validates the same bounded HTTP(S) contract in OpenAPI, Go and the editor, preserves local state on revision conflicts, and uses native accessible preview controls. The editor route still owns too much cross-panel orchestration. | Reduce route-level orchestration into focused typed hooks/controllers without turning the editor into one giant form or duplicating draft ownership. |
