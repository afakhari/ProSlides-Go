...
| P2 | Question, content and presentation design authoring now use typed domain drafts shared by their inspectors and preview surfaces. Unsaved state projects immediately into theme/contrast-safe question, content, leaderboard and slide-list previews; discard returns to persisted server state and stale-revision conflicts preserve local drafts until explicitly resolved. Audio still contains larger custom draft/dirty logic. | Apply the same domain-owned pattern to audio where it reduces real complexity, then reduce route-level orchestration without turning the editor into one giant form. |
...
| P2 | Browser coverage now includes question-authoring save/reorder/Persian-number/edit-conflict recovery, content-authoring live preview/discard/save/conflict recovery, and presentation-design contrast-safe preview/discard/save/conflict recovery on the real API stack; focused component/API-state coverage is still thinner than protocol coverage. | Add Vitest + Testing Library + MSW for dense pending/success/validation/conflict/cancellation states where browser tests would be too slow or broad. |
