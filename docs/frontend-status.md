...
| P2 | Question, content, presentation design and presentation audio use typed domain drafts with explicit baseline/dirty/save/discard ownership. Editor panel confirmation, unsaved-browser protection, transient notices and responsive overlay locking are now delegated to focused typed hooks, but the legacy route still owns slide CRUD/type-selection mutation orchestration. | Extract slide mutation/type-selection orchestration behind typed module boundaries, then convert the thinner route boundary to TSX without duplicating draft ownership or creating a giant form. |
