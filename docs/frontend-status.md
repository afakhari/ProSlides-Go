...
| P2 | Question authoring now uses one typed domain draft/reducer shared by the inspector and live editor canvas. Unsaved text, timing, scoring, option order, correctness and media project immediately into a theme/contrast-safe preview; discard falls back to persisted server state. Other editor surfaces still contain large custom draft/dirty logic. | Extend the same domain-owned pattern to content/design/audio where it reduces real complexity, then converge those previews without turning the editor into one giant form. |
