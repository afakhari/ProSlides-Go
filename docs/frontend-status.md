...
| P1 | Live protocol/reconnect/roster state remains heavily embedded in React context. The duplicate event-to-server-data synchronization layer has been removed, so projections now derive directly from the authoritative snapshot/roster. | Extract a typed `modules/live/api + runtime + react` boundary and then move the remaining legacy live contexts/hooks under that module without changing protocol semantics. |
