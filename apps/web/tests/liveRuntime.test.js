...
test("presence updates keep score ordering while the manager is on a leaderboard", async () => {
  let onEvent = null;
  const rosterOrders = [];
  const runtime = createLiveRuntime("manager", {
    storage: null,
    transport: {
      ...
      createLiveSession: async () => managerSession("session", "leaderboard", 2),
      getLiveSnapshot: async () =>
  ...
  assert.equal(await runtime.connect("presentation"), true);
  assert.deepEqual(rosterOrders, ["score"]);
  ...
  for (let index = 0; index < 20 && rosterOrders.length < 2; index += 1) {
    await new Promise((resolve) => setTimeout(resolve, 0));
  }

  assert.deepEqual(rosterOrders, ["score", "score"]);
  runtime.destroy();
});
