...
type PlayerContentSlideProps = {
  quiz: LivePresentationModel;
  content: LegacyContentSlide;
};

export function PlayerContentSlide({
  quiz,
  content,
}: PlayerContentSlideProps) {
  const { isConnected } = useLiveSession();
  const title = content.title || "اسلاید توضیحی";
    ...
    <ParticipantShell quiz={quiz} connected={isConnected} showConnection>
      <article className="flex flex-1 flex-col justify-center py-5 text-center">
        <div className="rounded-[2rem] border border-[color:var(--live-border)] bg-[color:var(--live-surface)] p-5 shadow-2xl backdrop-blur-xl sm:p-9">
          ...
          <h1 className="text-3xl font-black leading-tight sm:text-4xl" dir="auto">
            {title}
          </h1>
          {image ? (
            <img
          ...
          <div
            ...
            role="status"
          >
            برای ادامه، منتظر ارائه‌دهنده بمانید
          </div>
        </div>
      </article>
    </ParticipantShell>
