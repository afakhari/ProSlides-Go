...
export function PlayerLeaderBoard({ quiz }: PlayerLeaderBoardProps) {
    ...
    <ParticipantShell quiz={quiz} connected={isConnected} showConnection>
      <section className="flex flex-1 flex-col justify-center py-5 text-center">
        <Motion.div
          ...
          className="rounded-[2rem] border border-[color:var(--live-border)] bg-[color:var(--live-surface)] p-6 shadow-2xl backdrop-blur-xl sm:p-10"
        >
          ...
          <div className="mx-auto my-7 grid h-36 w-36 place-items-center rounded-full border-4 border-white/25 bg-white/10 shadow-2xl">
            <div>
              ...
              <p className="text-5xl font-black">
                {rank == null ? "—" : rank.toLocaleString("fa-IR")}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-[color:var(--live-border)] bg-white/10 p-4">
              <p className="text-xs text-[color:var(--live-muted)]">امتیاز شما</p>
          ...
          <p className="mt-7 text-sm leading-7 text-[color:var(--live-muted)]">
            مرحله بعدی به‌محض اقدام ارائه‌دهنده در همین صفحه نمایش داده می‌شود.
          </p>
        </Motion.div>
      </section>
    </ParticipantShell>
