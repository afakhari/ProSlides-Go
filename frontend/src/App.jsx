...
const App = () => {
  ...
  const players = [
    ...
    { user_id: 1, name: "Chloe",character: "👑",color: "#db2777",rank:1, total_points: 153, new_points: 61,   },
    { user_id: 2, name: "Trang",character: "🌸",color: "#059669",rank:2, total_points: 149, new_points: 49,   },
    { user_id: 3, name: "Alex",character: "🐱",color: "#65a30d",rank:3, total_points: 34, new_points: 34, },
    { user_id: 4, name: "Jenny",character: "🧁",color: "#2563eb",rank:5, total_points: 0, new_points: 0, },
    { user_id: 5, name: "Kian",character: "😂",color: "#4563bb",rank:4, total_points: 20, new_points: 20, },
  ];
  ...
  useEffect(() => {
    const withOld = players.map((p) => ({ ...p, oldScore: p.total_points - p.new_points }));
    const sortedOld = [...withOld].sort((a, b) => b.oldScore - a.oldScore);
    setDisplayedPlayers(sortedOld);

    const t = setTimeout(() => {
      const sortedNew = [...players].sort((a, b) => b.total_points - a.total_points);
      ...
      <main>
        <section className="p-4">
          ...
          <div className="text-center">
            ...
            <p className="text-white/70 text-lg mt-2">{players.length} players</p>
          </div>
              ...
              {displayedPlayers.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className="relative overlay-hidden bg-white/10 w-full h-14 mr-3"
                    >
                      {/* Colored fill */}
                      <motion.div
                      ...
                      <div className="relative z-20 flex items-center px-4 py-3 gap-4">
                        <div className="player-avatar text-2xl">{p.character}</div>
                    ...
                    <div className="relative w-[10%] text-white font-semibold ml-3">
                      {p.total_points}p <span className="text-white/60 text-sm">+{p.new_points}</span>
                    </div>
                  </motion.li>
                );
              })}
